...
One final thing I added for readibility, is the use of packed structs for pipeline registers data input and outputs. This will make pipeline stages more readable, and easier to update.
...
- Made pipeline registers inputs and outputs into packed struct types

## September 27th

### Plan for the day:
Now that Dhrystone is implemented and running, I think it's best to look more into implementing the benchmarking performance features. These will better allow me to determine how different changes affect processor performance as I move forward with the design, in addition to the synthesis results. Having synthesis information (fmax, area utilization, etc), and architectural information (IPC, cache misses, branch accuracy, etc.) will allow for better architectural decision making as I move forward and implement new features. So I will be looking into implementing more refined information gathering.

### Notes:
I spent some of the day getting my project setup in Vivado again, so I can take a look at the synthesis results. As of now, my clock period is sitting at **14.195ns**, or **70.447Mhz**, which I believe is still a drop from before the I-cache was implemented. I also found that (not including the dmem, as this will be changed), my design uses **12,236** LUTs, representing a utilization of **~70%**. To be honest, I would much rather these numbers give me more headroom, but it's what I'm currently working with. 

The reason I did this, was because I began looking into performance monitoring, and found that RISC-V uses CSR's to monitor performance. Initially I was planning to just use top-level testbench monitors to view performance, but now that I know this, I believe it is best to conform with the RISC-V spec, and implement csr instructions and registers. Anyways, the point is that this idea of implementing this got me thinking about how my design was synthesizing, and I wanted to see what I was working with, mostly area wise.

That all said, at 70% usage, I think I'm honestly pushing it considering I want to add a d-cache, and possibly other peripherals. That said, I think having these registes will be an interesting additon, and open up the architecture to further improvement. I recognize that "further improvement" may mean requiring a larger board, but again, I think implementing the basics for CSR operations will be interesting, and if kept bare bones, likely won't use up a massive amount of LUT's, or affect timing too heavily.

So I think I'm going to compromise. I will impelement CSR instruction decoding and support, and include the mcycle, and minstret registers. For other metrics that would usually stored in mhpmcounter<x>, I will use the testbench level monitor module style that I was planning on earlier. This compromise sets up CSR functionality that can be expanded on later, while not committing excessive area, and still allowing for more performance metrics to be measured.

The CSR instructions I plan on adding include:
- csrrw
- csrrs
- csrrc
- csrrwi
